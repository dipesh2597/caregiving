    ...
    def Request(self):
        ...
        for i in range(len(younger_id)):
            ...
            if i==(request_no-1) and request_no!=0:
                ...
                sql = f'UPDATE elders set elder_name = {elder_name[0]} WHERE PK_elder_id = {elder_id[0]}'
                val=(elder_name[0], elder_id[0])
                mycursor.execute(sql, val)
                # updating youngers table
                sql = f'UPDATE youngers set younger_name = {younger_name[0]} WHERE PK_elder_id = {younger_id[i][i]}'
